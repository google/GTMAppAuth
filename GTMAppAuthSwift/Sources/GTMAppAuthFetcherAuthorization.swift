...
/// An implementation of the @c GTMFetcherAuthorizationProtocol protocol for the
/// AppAuth library.
/// - note: Enables you to use AppAuth with the GTM Session Fetcher library.
@objc open class GTMAppAuthFetcherAuthorization: NSObject,
  ...
  /// - note: The provider name is just for allowing stored authorization
  /// to be associated with the authorizing service.
  @objc public let serviceProvider: String?
  /// User ID from the ID Token.
  /// - note: Never send this value to your backend as an authentication token,
  /// rather send an ID Token and validate it.
  @objc public let userID: String?
  /// The user email.
  @objc public let userEmail: String?
  /// The verified string. If the result is false, then the email address is
  /// listed with the account on the server, but the address has not been
  ...
  /// Delegate of the `GTMAppAuthFetcherAuthorization` used to supply
  /// additional parameters on token refresh.
  ...
  /// Creates a new `GTMAppAuthFetcherAuthorization` using the given
  /// `OIDAuthState` from AppAuth.
  /// - parameter authState: The authorization state.
  /// - parameter serviceProvider: An optional string to describe the service.
  /// - parameter userID: An optional string of the user ID.
  /// - parameter userEmail: An optional string of the user's email address.
  /// - parameter userEmailIsVerified: An optional string representation of a
  /// boolean to indicate that the email address has been verified. Pass "true"
  /// or "false".
  @objc public init(
  ...
  // MARK: - Authorizing Requests

  /// Adds an authorization header to the given request, using the authorization
  /// state. Refreshes the access token if needed.
  /// - parameter request: The request to authorize.
  /// - parameter handler: The block that is called after authorizing the
  /// request is attempted. If `error` is non-nil, the authorization failed.
  /// Errors in the domain `OIDOAuthTokenErrorDomain` indicate that the
  /// authorization itself is invalid, and will need to be re-obtained from the
  /// user. `GTMAppAuthFetcherAuthorization.Error` indicate another
  /// unrecoverable error. Errors in other domains may indicate a transitive
  /// error condition such as a network error, and typically you do not need to
  /// reauthenticate the user on such errors.
  /// - note: The completion handler is scheduled on the main thread, unless
  /// the `callbackQueue` property is set on the `fetcherService` in which case
  /// the handler is scheduled on that queue.
  ...
  /// state. Refreshes the access token if needed.
  /// - parameter request: The request to authorize.
  /// - parameter delegate: The delegate to receive the callback.
  /// - parameter sel: The `Selector` to call upon the provided `delegate`.
  @objc public func authorizeRequest(
    _ request: NSMutableURLRequest?,
    delegate: Any,
...
"""
  ...
  private func invokeAsyncCallback(with arguments: AuthorizationArguments) {
  ...
  /// Stops authorization for the provided `URLRequest` if it is queued for
  /// authorization.
  ...
  /// Returns `true` if the provided `URLRequest` is currently in the process
  /// of, or is in the queue for, authorization.
  ...
  /// Returns `true` if the provided `URLRequest` has the "Authorization" header
  /// field.
  ...
  /// - note: This doesn't guarantee that a request will get a valid
  /// authorization, as the authorization state could become invalid on the next
  /// token refresh.
  ...
  /// - returns: `false` if the `OIDAuthState`'s `refreshToken` is nil. `true`
  /// otherwise.
  /// - note: If `true`, calling this method will `setNeedsTokenRefresh()` on
  /// the `OIDAuthState` instance property.
  ...
  /// - returns: An `OIDServiceConfiguration` object setup with Google OAuth
  /// endpoints.
